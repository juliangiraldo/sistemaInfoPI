...
<?php $form=$this->beginWidget('CActiveForm', array(
    ...
    <div class="column">
        ...
        echo $form->labelEx(Pais::model(), 'id_pais'); ?>
        <?php
        $pais = new CDbCriteria;
        $pais->order = 'descripcion ASC';

        echo $form->dropDownList(Pais::model(), 'id_pais', CHtml::listData(Pais::model()->findAll(), 'id_pais', 'descripcion'), array(
                'ajax' => array(
                    'type' => 'POST',
                    'url' => CController::createUrl('lugares/departamentos'),
                    'update' => '#Departamento_id_departamento'
                ), 'prompt' => 'Seleccione un país...'
            )
        );
        ?>
        <?php echo $form->error(Pais::model(), 'id_pais'); ?>
    </div>

    <div class="column">
        <?php echo $form->labelEx(Departamento::model(), 'id_departamento'); 

        $departamento = new CDbCriteria;
        $departamento->order = 'descripcion ASC';

        echo $form->dropDownList(Departamento::model(), 'id_departamento', array(), array(
                'ajax' => array(
                    'type' => 'POST',
                    'url' => CController::createUrl('lugares/Ciudad'),
                    'update' => '#Paciente_id_lugar_nac'
                ), 'prompt' => 'Seleccione...'
            )
        );
        echo $form->error(Departamento::model(), 'id_departamento'); ?>
    </div>
    <div class="column">
        <?php 
	...
	<div class="row">
        ...
        <?php echo $form->dropDownList($model,'id_barrio', CHtml::listData(Barrio::model()->findAll(), 'id_barrio', 'descripcion'), array('empty'=>'Seleccione..')); ?>		
